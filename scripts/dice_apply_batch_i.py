#!/usr/bin/env python3
"""
Dice Easy Apply — Batch I (5 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_i.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_I_jobs.json"
RESULTS_FILE = LOG_DIR / "dice_batchI_results.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchI_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchI")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch I -- 5 Jobs for Vamsi M")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("FINAL REPORT -- BATCH I")
    logger.info("=" * 60)
