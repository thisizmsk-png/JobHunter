#!/usr/bin/env python3
"""
Dice Easy Apply — Batch L (16 jobs) for Vamsi M.
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_L_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchL_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchL")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch L -- 16 Jobs for Vamsi M")
    ...
    # Save results JSON
    results_file = LOG_DIR / "dice_batchL_results.json"
