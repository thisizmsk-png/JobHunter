#!/usr/bin/env python3
"""
Dice Easy Apply — Batch R (4 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_r.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_R_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchR_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchR")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch R -- 4 Jobs for Vamsi M")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("FINAL REPORT -- BATCH R")
    logger.info("=" * 60)
    ...
    # Save results JSON
    results_file = LOG_DIR / "dice_batchR_results.json"
