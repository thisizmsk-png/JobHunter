#!/usr/bin/env python3
"""
Dice Easy Apply — Batch J (7 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_j.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_J_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchJ_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchJ")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch J -- 7 Jobs for Vamsi M")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("FINAL REPORT -- BATCH J")
    logger.info("=" * 60)
    ...
    # Save results JSON
    results_file = LOG_DIR / "dice_batchJ_results.json"
