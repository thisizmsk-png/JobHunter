#!/usr/bin/env python3
"""
Dice Easy Apply — Batch F (30 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_F.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_F_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchF_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchF")
...
# Title-level skip patterns (case-insensitive)
TITLE_EXCLUDE = ["lead", "architect", "principal", "director", "manager", "vp", "chief"]
TITLE_SKIP_PHRASES = [
    "no h1b", "no c2c", "no opt", "no cpt", "citizens only", "gc only",
    "visa independent", "w2 only", "only w2", "only citizen",
    "no corp to corp", "fulltime", "full-time", "full time",
]
...
C2C_EXCLUSION_PHRASES = [
    ...
    "no h1b", "no visa", "no sponsorship", "security clearance",
    "no h-1b", "no h1-b",
]
...
def title_skip(title):
    """Return skip reason if title should be skipped, else None."""
    t = title.lower()
    for exc in TITLE_EXCLUDE:
        # Match word boundaries roughly
        if exc in t.split() or f" {exc} " in f" {t} " or t.startswith(exc + " ") or t.endswith(" " + exc):
            return f"Title contains '{exc}'"
    for phrase in TITLE_SKIP_PHRASES:
        if phrase in t:
...
def is_location_ok(title, desc_text):
    """Check if job location is acceptable (NJ, NY, or remote)."""
    combined = (title + " " + desc_text).lower()
    # If remote mentioned, always OK
    if any(w in combined for w in ["remote", "work from home", "wfh", "telecommute"]):
        return True
    # If NJ or NY mentioned, OK
    if any(w in combined for w in [" nj", "new jersey", " ny", "new york", "jersey city",
                                    "newark", "hoboken", "plainfield", "edison", "princeton",
                                    "manhattan", "brooklyn", "queens"]):
        return True
    # If no specific location restriction found, assume OK (many listings don't specify)
    return True
...
def check_job(job):
    ...
    # Check for Easy Apply button/link
...
def apply_job(job):
    """Navigate to Dice wizard and complete application."""
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch F -- 30 Jobs for Vamsi M")
    ...
    title_skipped = []
    for job in all_jobs:
        reason = title_skip(job["title"])
        if reason:
            title_skipped.append({"title": job["title"], "company": job["company"], "reason": reason})
            logger.info(f"  TITLE SKIP: {job['title']} @ {job['company']} -- {reason}")
        else:
            eligible_jobs.append(job)

    logger.info(f"\nTitle pre-screen: {len(title_skipped)} skipped, {len(eligible_jobs)} eligible")
    logger.info("")

    conn = get_db()
    results = []
    stats = {"total": len(all_jobs), "applied": 0, "already_applied": 0,
             "skipped_title": len(title_skipped), "skipped_desc": 0, "failed": 0}
    ...
    if title_skipped:
        logger.info(f"TITLE-SKIPPED ({len(title_skipped)}):")
        for r in title_skipped:
            logger.info(f"  {r['title']} @ {r['company']} -- {r['reason']}")
        logger.info("")
    ...
    results_file = LOG_DIR / f"dice_batchF_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(results_file, "w") as f:
        json.dump({"stats": stats, "title_skipped": title_skipped, "results": results}, f, indent=2)
