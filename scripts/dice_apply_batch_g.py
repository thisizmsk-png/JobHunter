#!/usr/bin/env python3
"""
Dice Easy Apply -- Batch G (32 jobs after pre-skip) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_g.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_G_jobs.json"
RESULTS_FILE = LOG_DIR / "dice_batchG_results.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchG_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchG")
...
# Pre-skip GUIDs (from user instructions)
PRE_SKIP_GUIDS = {
    "df2efdec-95a6-4267-a7a8-ba0afb09bd60",  # Senior Java Developer @ Madison WI - W2 /1099
    "3b0d84c9-a698-4d8e-b83e-06081ec9107d",  # Sr. Java API Developer...Only locals (TN)
    "f6a791cc-b6eb-4c4a-8207-10882b64579b",  # Need Java Developer...Hybrid - CA or GA
    "c7fcf3d4-02d0-43c9-819e-63914f2792d6",  # Java Developer- W2 Local only Chicago
    "6ff9d187-6995-4a1e-8779-f19149e7a27f",  # Java Backend Services Developer IV (Only W2, No Third party)
    "0b25699d-ebd1-4a11-b22b-034a21732aac",  # Sr. Java Fullstack Developer (Must local to Los Angeles, CA)
    "8cf0e769-e3e0-4a07-8e16-7f28d91e82e3",  # Only W2 - Java developer...Strongsville, OH
    "785a7a24-927d-44d2-965b-b680310291c6",  # Java Developer at Charlotte, NC...W2 only
}
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch G -- Jobs for Vamsi M")
    ...
    for job in all_jobs:
        guid = job.get("guid", "")
        if guid in PRE_SKIP_GUIDS:
            reason = f"Pre-skip (user instruction): {job['title']}"
        ...
        if TITLE_EXCLUDE.search(job["title"]):
            reason = f"Title exclusion: '{job['title']}' matches lead/architect/principal/director/manager/vp/chief"
            logger.info(f"  PRE-SKIP: {reason}")
            pre_skipped.append({
                "title": job["title"], "company": job["company"],
                "url": job["url"], "reason": reason,
    ...
    logger.info(f"Pre-filtered: {len(pre_skipped)} skipped, {len(eligible_jobs)} to process")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("FINAL REPORT -- BATCH G")
    logger.info("=" * 60)
    logger.info(f"Total jobs in batch: {stats['total']}")
    logger.info(f"Pre-skipped:         {stats['pre_skipped']}")
    ...
    # Save results to the fixed path requested by user
