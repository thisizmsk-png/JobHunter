#!/usr/bin/env python3
"""
Dice Easy Apply — Batch N (4 jobs) for Vamsi M.
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_N_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchN_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchN")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch N -- 4 Jobs for Vamsi M")
    ...
    # Save results JSON
    results_file = LOG_DIR / "dice_batchN_results.json"
