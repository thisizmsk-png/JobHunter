#!/usr/bin/env python3
"""
Dice Easy Apply — Batch D (30 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Pre-screens titles for:
  - Lead/Architect/Principal/Director/Manager/VP/Chief exclusions
  - "No H1" / "No Visa" in title
  - "Local to <non-NJ/NY>" in title
  - "W2 Only" / "W2 Profiles" in title

Then checks job description for C2C/W2/visa exclusions before applying.

Usage:
    python3 scripts/dice_apply_batch_D.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_D_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchD_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchD")
...
# Title-level exclusion patterns (case-insensitive)
TITLE_EXCLUDE_PATTERNS = re.compile(
    r'\b(lead|architect|principal|director|manager|vp|chief)\b', re.IGNORECASE
)

# Phrases in job description that disqualify
C2C_EXCLUSION_PHRASES = [
    "w2 only", "no c2c", "no h1b", "no visa", "no sponsorship",
    "no corp to corp", "no corp-to-corp", "citizens only",
    "gc only", "green card only", "us citizens only",
    "citizen only", "only w2", "w-2 only", "no third party",
]


def title_skip_reason(title, location):
    """Return skip reason if title should be excluded, else None."""
    t = title.lower()

    # Title role exclusions
    if TITLE_EXCLUDE_PATTERNS.search(t):
        return f"Title exclusion (role): {title}"

    # No H1 / No visa in title
    if "no h1" in t or "no visa" in t or "no g.c" in t:
        return f"Title says no H1/visa: {title}"

    # W2 in title
    if "w2 role" in t or "w2 profiles" in t or "w2 only" in t:
        return f"Title says W2 only: {title}"

    # "Local to <place>" where place is not NJ/NY and not remote
    local_match = re.search(r'local\s+to\s+(\w+)', t)
    if local_match:
        local_area = local_match.group(1).lower()
        if local_area not in ("nj", "new", "jersey", "ny", "york"):
            return f"Local only ({local_area}), candidate in NJ"

    # "Only local" or "need local" with non-NJ/NY location
    if ("only local" in t or "need local" in t or "local only" in t) and location:
        loc = location.lower()
        if "new jersey" not in loc and "new york" not in loc and "remote" not in loc:
            return f"Local only to {location}, candidate in NJ"
...
def check_job(job):
    ...
    # Check for "no h1b" / "no sponsorship" in description
    if desc_text:
        dl = desc_text.lower()
        if "no h1b" in dl or "no h1-b" in dl or "no h-1b" in dl:
            return {"eligible": False, "reason": "No H1B in description"}
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply — Batch D (30 Jobs) for Vamsi M")
    logger.info(f"Total jobs in batch: {len(all_jobs)}")
    logger.info(f"Log: {log_file}")
    logger.info("=" * 60)

    conn = get_db()
    results = []
    stats = {"total": len(all_jobs), "applied": 0, "already_applied": 0,
             "skipped_title": 0, "skipped_desc": 0, "failed": 0}

    # Pre-screen titles
    eligible_jobs = []
    for job in all_jobs:
        reason = title_skip_reason(job["title"], job.get("location", ""))
        if reason:
            logger.info(f"  TITLE-SKIP: {reason}")
            stats["skipped_title"] += 1
            results.append({
                "url": job["url"], "title": job["title"],
                "company": job["company"],
                "result": "skipped_title", "reason": reason
            })
        else:
            eligible_jobs.append(job)

    logger.info(f"\nTitle pre-screen: {len(eligible_jobs)} eligible, "
                f"{stats['skipped_title']} skipped")
    logger.info("=" * 60)
    ...
    for i, job in enumerate(eligible_jobs):
        ...
        if not detail["eligible"]:
            ...
            if reason == "Already applied":
                ...
                results.append({
                    "url": url, "title": title, "company": company,
                    "result": "skipped_desc", "reason": reason
                })
            continue
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("FINAL REPORT — Batch D")
    logger.info("=" * 60)
    ...
    logger.info(f"Skipped (title):    {stats['skipped_title']}")
    logger.info(f"Skipped (desc):     {stats['skipped_desc']}")
    ...
    results_file = LOG_DIR / f"dice_batchD_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
