#!/usr/bin/env python3
"""
Dice Easy Apply — Batch M (4 jobs) for Vamsi M.
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_M_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchM_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchM")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch M -- 4 Jobs for Vamsi M")
    ...
    # Save results JSON
    results_file = LOG_DIR / "dice_batchM_results.json"
