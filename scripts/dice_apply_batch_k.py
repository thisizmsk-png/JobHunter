#!/usr/bin/env python3
"""
Dice Easy Apply — Batch K (18 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_k.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_K_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchK_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchK")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch K -- 18 Jobs for Vamsi M")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("FINAL REPORT -- BATCH K")
    logger.info("=" * 60)
