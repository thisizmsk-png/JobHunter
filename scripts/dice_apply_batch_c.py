#!/usr/bin/env python3
"""
Dice Easy Apply — Batch C (30 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_c.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_C_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchC_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchC")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch C -- 30 Jobs for Vamsi M")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("FINAL REPORT -- BATCH C")
    logger.info("=" * 60)
    ...
    results_file = LOG_DIR / f"dice_batchC_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
