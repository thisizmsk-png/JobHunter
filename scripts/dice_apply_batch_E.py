#!/usr/bin/env python3
"""
Dice Easy Apply — Batch E (30 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_E.py
"""
import hashlib
import json
import logging
...
log_file = LOG_DIR / f"dice_batchE_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchE")
...
# Load jobs from batch_E_jobs.json
BATCH_FILE = PROJECT_ROOT / "data" / "batch_E_jobs.json"
with open(BATCH_FILE) as f:
    ALL_JOBS = json.load(f)

# Title-level exclusion patterns
TITLE_EXCLUDE = ["lead", "architect", "principal", "director", "manager", "vp", "chief"]

# Title-level W2/citizenship pre-skips (clear from title alone)
TITLE_W2_SKIP = ["only w2", "w2 only", "citizens only", "gc only", "citizen & gc",
                  "only citizen", "only gc"]
...
# ---------------------------------------------------------------------------
# Pre-filter jobs by title
# ---------------------------------------------------------------------------

def title_prefilter(job):
    ...
    # Check W2-only / citizenship from title
    for phrase in TITLE_W2_SKIP:
        if phrase in title_lower:
            return False, f"Title contains '{phrase}' - not C2C eligible"
...
def check_job(job):
    """Navigate to job detail, check eligibility using the description element."""
    url = job["url"]
    ...
    if has_apply != "true":
        ...
        return {"eligible": False, "reason": "No Easy Apply button found"}
...
def main():
    conn = get_db()
    results = []
    stats = {"total": len(ALL_JOBS), "applied": 0, "already_applied": 0, "skipped": 0, "failed": 0}

    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch E -- 30 Jobs for Vamsi M")
    logger.info(f"Target jobs: {len(ALL_JOBS)}")
    logger.info(f"Log: {log_file}")
    logger.info("=" * 60)

    # Title pre-filter pass
    eligible_jobs = []
    for job in ALL_JOBS:
        passes, reason = title_prefilter(job)
        if not passes:
            logger.info(f"  PRE-SKIP: {job['title']} @ {job.get('company', '?')} -- {reason}")
            stats["skipped"] += 1
            results.append({
    ...
    logger.info(f"\nPre-filter: {len(ALL_JOBS)} total, {len(eligible_jobs)} eligible, {stats['skipped']} skipped by title")

    # Verify Chrome is accessible and logged in
    logger.info("\nVerifying Dice login...")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("BATCH E FINAL REPORT")
    logger.info("=" * 60)
    ...
    results_file = LOG_DIR / f"dice_batchE_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
