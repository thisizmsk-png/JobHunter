#!/usr/bin/env python3
"""
Dice Easy Apply — Batch P (4 jobs) for Vamsi M.
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_P_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchP_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchP")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch P -- 4 Jobs for Vamsi M")
    ...
    # Save results JSON
    results_file = LOG_DIR / "dice_batchP_results.json"
