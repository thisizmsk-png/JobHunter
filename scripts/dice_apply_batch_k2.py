#!/usr/bin/env python3
"""
Dice Easy Apply — Batch K2 (13 jobs) for Vamsi M.
Uses AppleScript to control Chrome with existing logged-in session.

Usage:
    python3 scripts/dice_apply_batch_k2.py
"""
import hashlib
import json
import logging
...
BATCH_FILE = PROJECT_ROOT / "data" / "batch_K2_jobs.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
log_file = LOG_DIR / f"dice_batchK2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logger = logging.getLogger("dice_batchK2")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Dice Easy Apply Batch K2 -- 13 Jobs for Vamsi M")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("FINAL REPORT -- BATCH K2")
    logger.info("=" * 60)
    ...
    # Save results JSON
    results_file = LOG_DIR / "dice_batchK2_results.json"
